...
# Houston
# min_lat, max_lat, min_lon, max_lon = 27.8, 29.8, -95.5, -93.5
min_lat, max_lat, min_lon, max_lon = 28.8, 29.8, -95.25, -94.25
...
class displacement_error(nn.Module):
    ...
    def get_features(self, prediction):
        lat_1 = prediction[..., 0].unsqueeze(-1).expand(
            prediction.size(0), prediction.size(1), prediction.size(2), prediction.size(1))
        lon_1 = prediction[..., 1].unsqueeze(-1).expand(
            prediction.size(0), prediction.size(1), prediction.size(2), prediction.size(1))
...
def scale_values(lat, lon):
    ...
    lon = (max_lon-min_lon) * lon + min_lon
    return lat, lon
...
def equirectangular_distance2(x, y):
    lat1 = x[..., 0]
    lon1 = x[..., 1]
    lat2 = y[..., 0]
    lon2 = y[..., 1]
    ...
    dlon = lon2-lon1
    dlat = lat2-lat1
    dist = (dlat) ** 2 + (dlon * math.cos((lat1 + lat2) / 2)) ** 2
    dist = radius_earth * math.sqrt(dist + (1e-24))
    return dist